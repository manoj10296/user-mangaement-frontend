...
import { usersigninInitiate } from '../../../redux/actions/auth';
...
const Login = (props) => {
  ...
  const onSubmit = (e) => {
    e.preventDefault()
    dispatch(usersigninInitiate(state))
  }
        ...
        <ColumneTwo>
          <FormContainer>
            <FormComponent className="ui form">
              ...
              <ButtonComponent onClick={onSubmit}>  SIGN IN {authState.loginLoader && <i className="fa fa-spinner fa-spin"></i>} </ButtonComponent>
            </FormComponent>
          </FormContainer>
          <RegisterText onClick={() => props.history.push('/register')}>Don't have an account. Click here to register</RegisterText>
        </ColumneTwo>
      </Container>
    </MainDiv>
  )
